...
class Solution
{
    ...
    static int[] dijkstra(int v, ArrayList<ArrayList<ArrayList<Integer>>> adj, int s)
    {
        ...
        each vertex has a list n of lists.
        each list l in n is a vertex-weight pair.
        the input format: a, b, w.
        a and b are the edges. w is the weight
        adj[0] would be a list of vertex, weight pairs
        adj[0][0] would be the first vertex and its weight. A list
        of two values. */
        
            int[] distances = new int[v];
            Arrays.fill(distances, Integer.MAX_VALUE);
            IndexedPriorityQueue ipq = new IndexedPriorityQueue(v);
            Set<Integer> visited = new HashSet<Integer>(v);
            ipq.insert(s, 0);
            //System.err.println("inserted "+s);
    		int vertex, weight, currKey, currWeight;
            while (!ipq.empty()){
    			currWeight = ipq.peekValue();
    			
                currKey = ipq.poll();
                //System.err.println("currKey: "+currKey+"weight: "+currWeight);
                distances[currKey] = currWeight;
                //System.err.println("weight: "+  currWeight);
    			visited.add(currKey);
                // get curr's adjacency list
                ArrayList<ArrayList<Integer>> neighbours = adj.get(currKey);
                for (ArrayList<Integer> neighbour : neighbours){
    				vertex = neighbour.get(0);
    				weight = neighbour.get(1);
                    //System.err.println("vertex: " + vertex);
                    //System.err.println("weight: " + weight);
    				if (weight == 0 || vertex == currKey){
    				    // this is an 'infinity' edge or self-loop.
    				    continue;
    				}
                    // check if it's been visited
                    if (!visited.contains(vertex)){
    					// if ipq already contains the key, update if needed
    					if (ipq.contains(vertex) && 
    					ipq.getValue(vertex) > weight + currWeight){
    					    ipq.update(vertex, weight + currWeight);
    					}
    					
    					// if the ipq doesn't have it, add it to the queue.
    					if (!ipq.contains(vertex)){
    					    ipq.insert(vertex, weight+currWeight);
    					  /*  System.err.println("added vertex "+vertex+
    					    " with weight "+(weight+currWeight)); */
    					}
                    }
                }
            }
